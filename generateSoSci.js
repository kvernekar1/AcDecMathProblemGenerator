...
const socialScience = await new PDFLoader('Social-Science-Resource-Guide.pdf').load();
...
for (const chunk of socialScience) {
  if (page > 82) {
    break;
  }
  if (page < 5) {
  ...
  const prompt = promptTemplate
    .replace('{subject}', 'Social Science')
  ...
  try {
    ...
    for (const item of data) {
      ...
      rows.push([q, a, b, c, d, e, ans, expl, 'Social Science', section]);
    }
